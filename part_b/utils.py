...
def load_student_csv(root_dir="/data"):
    """Load the student metadata as a list of dictionaries.
    :param root_dir: str
    :return: A dictionary {user_id: list, gender: list, date_of_birth: list, premium_pupil: list}
    """
    path = os.path.join(root_dir, "student_meta.csv")
    if not os.path.exists(path):
        raise Exception("The specified path {} does not exist.".format(path))

    data = {"user_id": [], "gender": [], "date_of_birth": [], "premium_pupil": []}
    with open(path, "r") as csv_file:
        reader = csv.DictReader(csv_file)
        for row in reader:
            try:
                data["user_id"].append(int(row["user_id"]))
                data["gender"].append(int(row["gender"]))
                data["date_of_birth"].append(
                    datetime.strptime(row["data_of_birth"], "%Y-%m-%d %H:%M:%S.%f")
                    if row["data_of_birth"]
                    else None
                )
                data["premium_pupil"].append(
                    float(row["premium_pupil"]) if row["premium_pupil"] else None
                )
